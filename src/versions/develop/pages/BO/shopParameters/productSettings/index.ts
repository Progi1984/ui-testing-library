 ...
 * @extends BOBasePage
 */
class BOProductSettingsPage extends BOBasePage implements BOProductSettingsPageInterface {
  ...
  private readonly productPageForm: string;
  ...
  private readonly displayNotificationIfProductInCartToggleInput: (toggle: number) => string;
  ...
  constructor() {
    ...
    this.displayAvailableQuantitiesToggleInput = (toggle: number) => `${this.productPageForm} #page_display_quantities_${toggle}`;
    this.displayAddToCartToggleInput = (toggle: number) => `${this.productPageForm
    } #page_allow_add_variant_to_cart_from_listing_${toggle}`;
    this.separatorAttributeOnProductPageSelect = `${this.productPageForm} #page_attribute_anchor_separator`;
    this.displayDiscountedPriceToggleInput = (toggle: number) => `${this.productPageForm} #page_display_discount_price_${toggle}`;
    this.displayNotificationIfProductInCartToggleInput = (toggle: number) => `${this.productPageForm
    } #page_display_amount_in_cart_${toggle}`;
    this.saveProductPageFormButton = `${this.productPageForm} #form-page-save-button`;
  ...
  */
   ...
   * @returns {Promise<string>}
   */
  async setMaxSizeOfSummaryValue(page: Page, size: number): Promise<string> {
  ...
  /**
   * Returns if "Display notifications if the product is already in the cart" is checked
   * @param page {Page} Browser tab
   * @returns {Promise<boolean>}
   */
  async isDisplayNotificationIfProductInCartChecked(page: Page): Promise<boolean> {
    return this.isChecked(page, this.displayNotificationIfProductInCartToggleInput(1));
  }

  /**
   * Set "Display notifications if the product is already in the cart"
   * @param page {Page} Browser tab
   * @param toEnable {boolean} True if we need to enable "Display notifications if the product is already in the cart"
   * @returns {Promise<string>}
   */
  async setDisplayNotificationIfProductInCartStatus(page: Page, toEnable: boolean = true): Promise<string> {
    await this.setChecked(page, this.displayNotificationIfProductInCartToggleInput(toEnable ? 1 : 0));
    await page.locator(this.saveProductPageFormButton).click();
    await this.elementNotVisible(page, this.displayNotificationIfProductInCartToggleInput(!toEnable ? 1 : 0), 2000);
...
module.exports = new BOProductSettingsPage();
